...
typedef enum {
    ...
    TYPE_INITIALIZER,
    TYPE_METHOD,
    // Scripts have an inplicit top level function with special handling
    // for globals.
    TYPE_SCRIPT
} FunctionType;

typedef struct Compiler {
    // Linked list of compilers used to handle functions compilation.
    // On the C stack thanks to resursive descent:
    // function() calls itself.
    struct Compiler* enclosing;
    ObjFunction* function; // Current function being built.
    FunctionType type;
...
static void initCompiler(Compiler* compiler, FunctionType type) {
    ...
    if (type != TYPE_SCRIPT) {
        current->function->name =
            copyString(gParser.previous.start,
                       gParser.previous.length);
    }

    // Stack slot zero is reserved for VM use only.
...
static void endScope() {
    ...
    while (current->localCount > 0 &&
        ...
        if (current->locals[current->localCount - 1].isCaptured) {
            ...
            // TODO(alex): OP_POPN to pop multiple slots at once.
            emitByte(OP_POP);
        }
        current->localCount--;
    }
...
// isLocal - Closure captures a local variable or an upvalue.
...
static int resolveUpvalue(Compiler* compiler, Token* name) {
    ...
    // For when a function closes over a variable that has already
    // left the stack.
...
static void markInitialized() {
    // There is no local variable to mark initialized in
    // the global scope.
    if (current->scopeDepth == SCOPE_GLOBAL) {
        return;
    }
    current->locals[current->localCount - 1].depth = current->scopeDepth;
}
...
static void dot(bool canAssign) {
    ...
    uint8_t name = identifierConstant(&gParser.previous);
...
static void function(FunctionType type) {
    ...
    // No need to close lingering scope, because we end the compiler.
...
static void method() {
    ...
    uint8_t constant = identifierConstant(&gParser.previous);

    FunctionType type = TYPE_METHOD;
    if (gParser.previous.length == 4 &&
        memcmp(gParser.previous.start, "init", 4) == 0) {
...
static void classDeclaration() {
    ...
    Token className = gParser.previous;
    uint8_t nameConstant = identifierConstant(&gParser.previous);
    declareVariable();
    ...
    if (match(TOKEN_LESS)) {
        ...
        if (identifiersEqual(&className, &gParser.previous)) {
...
static void funDeclaration() {
    ...
    markInitialized(); // To allow recursion.
...
ObjFunction* compile(const char* source) {
    ...
    ObjFunction* function = endCompiler();
    return gParser.hadError ? NULL : function;
}
