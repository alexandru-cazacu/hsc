...
static void initVM() {
    resetStack();
    gVM.objects = NULL;
    gVM.bytesAllocated = 0;
    gVM.nextGC = 1024 * 1024;
...
static bool vmCall(ObjClosure* closure, int argCount) {
...
static ObjUpvalue* captureUpvalue(Value* local) {
    ...
    ObjUpvalue* upvalue = gVM.openUpvalues;
    ...
    if (prevUpvalue == NULL) {
        gVM.openUpvalues = createdUpvalue;
    } else {
...
static void closeUpvalues(Value* last) {
    while (gVM.openUpvalues != NULL &&
            gVM.openUpvalues->location >= last) {
        ObjUpvalue* upvalue = gVM.openUpvalues;
